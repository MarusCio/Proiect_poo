...
class Joc {
    ...
    public:
        ...
        int Set_Dificultate();
        void Reset_Revolver(const std::vector<Player*>& players);
        bool Minte(Player& jucator_crt, Player& adversar, const Table& masa);
        std::vector<Player*> Jucatori_Activi(std::vector<Player*>& jucatori_initiali, Pachet_Carti& carti);
        void Incepe_Joc();
        [[nodiscard]] int Get_Lungime_Max() const;
